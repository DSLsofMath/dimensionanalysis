...
+ implemented using |Data.Float| and some postulates
...
\begin{code}
record RawField (S : Set) : Set where
  ...
  pow : S -> Integer -> S
  pow s (+ n) = s ^ n
  pow s (Integer.negsuc n) = 1s /s (s ^ (suc n))
...
+ Now, how to we type this? We clearly need to be able to multiply and
  divide dimensions, and we also need a value to denote
  "dimensionless".
*** DimStuff
\begin{code}
record DimStuff : Set1 where
  field
    Dim : Set
    Dimless : Dim
    _*d_ : Dim -> Dim -> Dim
...
\begin{code}
  ...
  d ^d +_ n = d ^dn n
  d ^d Integer.negsuc n = Dimless /d (d ^dn suc n)
\end{code}
...
scalars |S|.
\begin{code}
module Quantities (dimstuff : DimStuff) (S : Set) where
  open DimStuff dimstuff

  variable d d1 d2 : Dim
  record Qstuff : Set1 where
    field
      Q     : Dim -> Set
      dim   : Q d -> Dim
...
\begin{code}
module LTM (S : Set) (rf : RawField S) where
...
\begin{code}
  ...
  dimfun d su = foldr _ _*s_ 1s (zipWith pow su d)
\end{code}
